...
(function (SpringType) {
    SpringType[SpringType["STRUCTURAL"] = 0] = "STRUCTURAL";
    SpringType[SpringType["SHEAR"] = 1] = "SHEAR";
    SpringType[SpringType["BEND"] = 2] = "BEND";
})(SpringType || (SpringType = {}));
export class Spring {
    constructor(a, b, stiffness = 10.0, damping = 0.5, type = SpringType.STRUCTURAL) {
        this.broken = false;
        this.particleA = a;
        this.particleB = b;
    ...
    solveConstraint(compliance = 0.0) {
        if (this.broken || (this.particleA.isFixed && this.particleB.isFixed))
            return;
        const direction = this.particleB.position.copy().subtract(this.particleA.position);
        const currentLength = direction.length();
        if (currentLength === 0)
