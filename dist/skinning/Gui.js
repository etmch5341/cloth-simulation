...
export class GUI {
    ...
    constructor(canvas, animation) {
        this.statusBarHeight = 200; // Height of the status bar area
        this.previewWidth = 320; // Width of the new preview panel
        // Added properties for bone manipulation
        this.highlightedBoneIndex = -1; // Currently highlighted bone (-1 means none) //selected bone
        this.boneDragging = false; // Whether we're currently dragging a bone
        // private dragStartX: number = 0;        // Starting X coordinate for drag
        // private dragStartY: number = 0;        // Starting Y coordinate for drag
        this.boneRadius = 0.1; // Radius for bone picking
        this.hoverX = 0;
        this.hoverY = 0;
        // Keyframe properties
        this.keyframes = [];
        this.currentKeyframeTime = 0;
        this.keyframeInterval = 1.0; // 1 second per keyframe
        // preview fields
    ...
    scrollKeyframePreviews(direction) {
    ...
    reset() {
        ...
        this.currentKeyframeTime = 0;
        this.camera = new Camera(new Vec3([0, 0, -6]), new Vec3([0, 0, 0]), new Vec3([0, 1, 0]), 45, this.mainViewWidth / this.mainViewHeight, 0.1, 1000.0);
    }
    /**
     * Sets the GUI's camera to the given camera
     * @param cam a new camera
     */
    setCamera(pos, target, upDir, fov, 
    ...
    rayIntersectsBone(ray, bone, radius) {
