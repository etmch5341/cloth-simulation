...
import { 
  clothVSText, 
  clothFSText, 
  clothWireframeVSText, 
  clothWireframeFSText,
  springVSText,
  springFSText,
  pointVSText,
  pointFSText,
...
export class ClothAnimation extends CanvasAnimation {
  ...
  private cloth: Cloth;
  private clothRenderPass: RenderPass;
  ...
  private renderMode: RenderMode = RenderMode.SHADED;
  
  // Cloth physics parameters
  private timeStep: number = 1/60;
  private accumulator: number = 0;
  private clothWidth: number = 4;
  private clothHeight: number = 4;
  private clothRows: number = 15;
  private clothCols: number = 15;
  ...
  private meshTestEnabled: boolean = false;
  private meshTestTimer: number | null = null;
  private meshTestFaceCounts: number[] = [1, 4, 16, 64, 256, 1024, 4096];
  ...
  private initClothRenderPass(): void {
    ...
    this.clothRenderPass.addAttribute("vertPosition", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, meshData.positions);
    
    this.clothRenderPass.addAttribute("vertNormal", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, meshData.normals);
    
    this.clothRenderPass.addAttribute("vertUV", 2, this.ctx.FLOAT, false,
      2 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, meshData.uvs);
    
    this.clothRenderPass.addUniform("mWorld",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(Mat4.identity.all()));
      });
      
    this.clothRenderPass.addUniform("mView",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
      });
      
    this.clothRenderPass.addUniform("mProj",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
      });
      
    this.clothRenderPass.addUniform("lightPosition",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform4fv(loc, this.lightPosition.xyzw);
      });
      
    this.clothRenderPass.addUniform("materialColor",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform3fv(loc, new Float32Array([0.7, 0.7, 0.9])); // Cloth color
      });
      
    this.clothRenderPass.addUniform("ambientIntensity",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1f(loc, 0.2);
      });
      
    this.clothRenderPass.addUniform("diffuseIntensity",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1f(loc, 0.8);
      });
      
    this.clothRenderPass.addUniform("specularIntensity",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1f(loc, 0.5);
      });
      
    this.clothRenderPass.addUniform("shininess",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1f(loc, 32.0);
      });
      
    this.clothRenderPass.addUniform("cameraPosition",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform3fv(loc, this.gui.cameraPosition().xyz);
      });
      
    this.clothRenderPass.addUniform("useTexture",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
  ...
  private initWireframeRenderPass(): void {
    const meshData = this.cloth.generateMeshData();
    
    this.wireframeRenderPass.setIndexBufferData(meshData.indices);
    
    this.wireframeRenderPass.addAttribute("vertPosition", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, meshData.positions);
    
    this.wireframeRenderPass.addUniform("mWorld",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(Mat4.identity.all()));
      });
      
    this.wireframeRenderPass.addUniform("mView",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
      });
      
    this.wireframeRenderPass.addUniform("mProj",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
      });
  ...
  private initSpringRenderPass(): void {
    const springData = this.cloth.generateSpringMeshData();
    
    this.springRenderPass.setIndexBufferData(springData.indices);
    
    this.springRenderPass.addAttribute("vertPosition", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, springData.positions);
      
    this.springRenderPass.addAttribute("vertColor", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, springData.colors);
    
    this.springRenderPass.addUniform("mWorld",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(Mat4.identity.all()));
      });
      
    this.springRenderPass.addUniform("mView",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
      });
      
    this.springRenderPass.addUniform("mProj",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
      });
  ...
  private initPointRenderPass(): void {
    ...
    this.pointRenderPass.addAttribute("vertPosition", 3, this.ctx.FLOAT, false,
      3 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, positions);
      
    this.pointRenderPass.addAttribute("isFixed", 1, this.ctx.FLOAT, false,
      1 * Float32Array.BYTES_PER_ELEMENT, 0, undefined, isFixed);
    
    this.pointRenderPass.addUniform("mWorld",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(Mat4.identity.all()));
      });
      
    this.pointRenderPass.addUniform("mView",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.viewMatrix().all()));
      });
      
    this.pointRenderPass.addUniform("mProj",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniformMatrix4fv(loc, false, new Float32Array(this.gui.projMatrix().all()));
      });
      
    this.pointRenderPass.addUniform("pointSize",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform1f(loc, 10.0); // Size of points
      });
      
    this.pointRenderPass.addUniform("freeColor",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
        gl.uniform3fv(loc, new Float32Array([0.0, 0.5, 0.9])); // Color for free particles
      });
      
    this.pointRenderPass.addUniform("fixedColor",
      (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => {
  ...
  public setWindDirection(direction: Vec3): void {
    this.cloth.windDirection = direction.normalize();
  }
  
  public setFabricType(type: FabricType): void {
    this.fabricType = type;
    this.initCloth();
  }
  
  public cutCloth(rayOrigin: Vec3, rayDirection: Vec3): void {
    this.cloth.cutCloth(rayOrigin, rayDirection, 0.1);
  }
  
  public pinParticle(row: number, col: number, fixed: boolean): void {
  ...
  public runProgressiveMeshTest(): void {
    ...
    const runNextMeshTest = () => {
      if (!this.meshTestEnabled) return;
  ...
  private stopProgressiveMeshTest(): void {
