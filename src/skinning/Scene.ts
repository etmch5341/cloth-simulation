...
export function computeDualQuat(bone: Bone): DualQuat {
...
export function generateCylinderGeometry(
  ...
  radius: number = 0.05,
  segments: number = 8
): { positions: Float32Array, indices: Uint32Array, boneIndices: Float32Array } {
...
export class KeyFrame {
  // Map of bone indices to their rotations in quaternions
  private boneRotations: Map<number, Quat>;
  
  constructor() {
    this.boneRotations = new Map<number, Quat>();
  }
  
  // Store the rotation for a specific bone
  public setBoneRotation(boneIndex: number, rotation: Quat): void {
    this.boneRotations.set(boneIndex, rotation.copy());
  }
  
  // Get the rotation for a specific bone
  public getBoneRotation(boneIndex: number): Quat | undefined {
    return this.boneRotations.get(boneIndex);
  }
  
  // Check if this keyframe has a rotation for a specific bone
  public hasBoneRotation(boneIndex: number): boolean {
    return this.boneRotations.has(boneIndex);
  }
  
  // Get all bone indices with stored rotations
  public getBoneIndices(): number[] {
...
export class Bone {
  public parent: number;           // Parent bone index (-1 for root)
  public children: number[];       // Child bone indices
  public position: Vec3;           // Current position in world coordinates
  public endpoint: Vec3;           // Current endpoint in world coordinates
  public rotation: Quat;           // Current orientation in world coordinates
  public index: number = -1;       // Bone index in the skeleton array

  public T: Mat4; // rest offset
  public D: Mat4; // world transform
  public R: Mat4; // local rotation
  public U: Mat4; // undeformed

  public localPos: Vec4;
  public localEndpoint: Vec4;
  public worldTransform: Mat4; // Add this property for dual quaternion calculation

  public flipForwardAxis: boolean;
...
export class Mesh {
  ...
  public getBoneDualQuaternions(): Float32Array {
  ...
  public initializeMatrices(bone: Bone) {
    if (bone.localPos) {
...
public roll(boneIndex: number, angle: number, flip: boolean): void {
...
// }
  ...
  public rotate(boneIndex: number, rotMatrix: Mat4, isParent: boolean){
  ...
  public findClosestBone(rayOrigin: Vec3, rayDir: Vec3, radius: number = 0.1): number {
    let closestBone = -1;
  ...
  public compose(rotateMat: Mat4, parent: Vec3) {
  ...
  public captureKeyframe(): KeyFrame {
...
public applyKeyframe(keyframe: KeyFrame): void {
...
private updateChildTransform(boneIndex: number): void {
  const bone = this.bones[boneIndex];
  const parentBone = this.bones[bone.parent];
  ...
  public interpolateKeyframes(keyframe1: KeyFrame, keyframe2: KeyFrame, t: number): void {
