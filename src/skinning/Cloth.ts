...
export class Cloth {
  ...
  public windDirection: Vec3 = new Vec3([0, 0, 1]);
  public windStrength: number = 0;
  ...
  constructor(width: number, height: number, rows: number, cols: number, fabricType: FabricType = FabricType.COTTON) {
    this.width = width;
    this.height = height;
    this.rows = rows;
    this.cols = cols;
  ...
  private createParticleGrid(rows: number, cols: number): void {
    // Initialize the grid
    ...
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        ...
        if (i < this.rows - 1 && j < this.cols - 1) {
          this.springs.push(new Spring(
            current, 
            this.particles[i + 1][j + 1], 
            props.shearStiffness, 
            props.damping, 
            SpringType.SHEAR
          ));
        }
  ...
  public updatePBD(dt: number): void {
    // Apply external forces
    this.applyGravity();
  ...
  public solveConstraints(iterations: number): void {
  ...
  public cutCloth(rayOrigin: Vec3, rayDirection: Vec3, cutRadius: number = 0.1): void {
    const springsToRemove: Spring[] = [];
  ...
  private rayIntersectsSpring(rayOrigin: Vec3, rayDir: Vec3, spring: Spring, radius: number): boolean {
  ...
  public generateMeshData(): { positions: Float32Array, normals: Float32Array, indices: Uint32Array, uvs: Float32Array } {
  ...
  public generateSpringMeshData(): { positions: Float32Array, colors: Float32Array, indices: Uint32Array } {
    ...
    for (let i = 0; i < springCount; i++) {
      ...
      switch (spring.type) {
        case SpringType.STRUCTURAL:
          r = 1.0; g = 0.0; b = 0.0; // Red for structural
          break;
        case SpringType.SHEAR:
          r = 0.0; g = 1.0; b = 0.0; // Green for shear
          break;
        case SpringType.BEND:
          r = 0.0; g = 0.0; b = 1.0; // Blue for bend
          break;
      }
  ...
  private calculateNormals(positions: Float32Array, indices: Uint32Array, normals: Float32Array): void {
