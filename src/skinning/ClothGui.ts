...
export class GUI implements IGUI {
  ...
  private isDraggingParticle: boolean = false;
  private selectedParticleRow: number = -1;
  private selectedParticleCol: number = -1;
   ...
   */
  constructor(canvas: HTMLCanvasElement, animation: ClothAnimation) {
  ...
  private findClosestParticle(ray: Ray): { found: boolean, row: number, col: number, distance: number } {
    const cloth = this.animation.getCloth();
  ...
  private dragSelectedParticle(): void {
    if (!this.isDraggingParticle) return;
